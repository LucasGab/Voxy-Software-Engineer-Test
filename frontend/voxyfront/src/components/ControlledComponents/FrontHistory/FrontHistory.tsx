...
import { clearFrontWord, getFrontWords } from '@/connector/words';
import { Box, Flex, Spinner, Text } from '@chakra-ui/react';
import React from 'react'
import { useMutation, useQuery, useQueryClient } from 'react-query';

export default function FrontHistory() {
  const { data: history, isLoading } = useQuery(['words', 'front'], () => getFrontWords());
  const queryClient = useQueryClient();

  const mutationDelete = useMutation(clearFrontWord, {
    onSuccess: () => {
